...
async def test_seed_is_idempotent(tmp_path):
    ...
    try:
        first = seed_articles(na, nb)
        assert len(first) == 10, f"first seed returned {len(first)}"

        second = seed_articles(na, nb)
        assert len(second) == 0, f"re-seed produced {len(second)} spurious articles"
