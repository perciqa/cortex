...
def test_parse_args_defaults():
    args = parse_args(["--broker", "wss://localhost:7432", "--static",
                       "dist", "--registry", "reg.json"])
...
async def test_build_app_wires_broker_subscriber(tmp_path: Path):
    app, lifecycle = build_app(broker_url="wss://localhost:7432",
                               static_dir=tmp_path, registry_path=tmp_path / "r.json")
