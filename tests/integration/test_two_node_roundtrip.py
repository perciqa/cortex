...
async def test_two_node_roundtrip(tmp_path: Path) -> None:
    ...
    prov = Provenance(producer_agent="did:percq:agent:alpha-1",
    ...
    got_publish = any(e.get("type") == "publish" for e in broker_received)
    assert got_publish, "broker did not receive publish"
